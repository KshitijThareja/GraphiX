...
const CallgraphContext = createContext<CallgraphContextType | undefined>(undefined)
...
export function CallgraphProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    console.log("CallgraphContext updated:", { repoUrl, callgraphData, metrics })
    if (repoUrl) {
  ...
  return (
    <CallgraphContext.Provider value={{ repoUrl, setRepoUrl, callgraphData, setCallgraphData, metrics, setMetrics, clearCallgraphData }}>
      {children}
